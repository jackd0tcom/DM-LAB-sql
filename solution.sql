...
SELECT email FROM salespeople WHERE region LIKE '%Northwest%';

-- Problem 4:
-- Task: Write a query that shows the given name, surname, and email of all
-- salespeople in the 'Northwest' region.

SELECT email, first_name, last_name from salespeople WHERE region LIKE '%Northwest%';
...
SELECT common_name, price FROM melons WHERE price > 5.00 AND melon_type LIKE '%watermelon%';
...
SELECT SUM(subtotal) FROM orders WHERE salesperson_id is NULL;
